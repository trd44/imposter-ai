...
class DatabaseManager:
    ...
    def GetSystemPromptFromID(personality_id: int):
        """
        Returns (nickname, deserialized system prompt json,)
        Will return None if cannot find
        """
        db = get_db()
        system_prompt = db.execute('''
            SELECT nickname, system_prompt
            FROM personality
            WHERE id = ?
            ''', (personality_id,)).fetchone()
        
        if system_prompt is not None:
            ret = (system_prompt[0], DeserializeJson(system_prompt[1]))
        else:
            ret = None
        return ret
    ...
    def SavePersonality(personality_id: int, nickname: str, system_prompt):
        ...
        try:
            db.execute('''
                INSERT OR REPLACE INTO personality (id, nickname, system_prompt) 
                VALUES (?, ?, ?)
                ''', (personality_id, nickname, SerializeJson(system_prompt)))
            db.commit()
        except Exception as e:
            #TODO: Log errors. Print right now for testing...
            print(e)